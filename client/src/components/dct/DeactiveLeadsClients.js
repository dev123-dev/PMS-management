...
const DeactiveLeadsClients = ({
  ...
  const [leadType, setLeadType] = useState(
    location?.data?.leadType
      ? location?.data?.leadType
      : localStorage.getItem("leadType")
  ); //leadType to detemine if the call is from Leads or Clients
  ...
  const activateALeadOrClient = () => {
    activateDctLeadOrClient({
      theLeadOrClientId: selDctLeadClient?._id,
      leadType: leadType,
    });
          ...
          <div className="row col-lg-12 col-md-12 col-sm-12 col-12 no_padding">
            <div className=" col-lg-1 col-md-11 col-sm-10 col-10 ">
              <h4 className="heading_color">
                Deactive {leadType === "Leads" ? "Leads" : "Clients"}
              </h4>
            </div>
          </div>
          <div className="row">
                        ...
                        dctLeadsClients.map((dctLeadClient, idx) => {
                          return (
                            ...
                            >
                              ...
                              <td>
                                {dctLeadClient.dctLeadDeactiveReason
                                  ? dctLeadClient.dctLeadDeactiveReason
                                  : ""}
                              </td>
                              <td>
                                {dctLeadClient.dctLeadAssignedToName
                                  ? dctLeadClient.dctLeadAssignedToName
                                  : ""}
                              </td>
                              <td>
                                <img
                                  ...
                                  onClick={() =>
                                    onClickActivate(dctLeadClient, idx)
                                  }
        ...
        <Modal.Body>
          <div>
            <p>
              Activating Lead -{" "}
              <b>
                {selDctLeadClient && selDctLeadClient.companyName
                  ? selDctLeadClient.companyName
                  : ""}
              </b>
            </p>
