...
import { editPwd } from "../../actions/auth";

const EditPassword = ({
  auth: { isAuthenticated, user },
  editPwd,
  onChangePwdModalChange,
  staffChangePwddata,
  successResponse,
}) => {
  let passwrdTooltip = {
  ...
  const onSubmit = async (e) => {
    ...
    if (checkErrors(formData)) {
      const finalData = {
        password: password,
        userId: staffChangePwddata._id,
      };
      editPwd(finalData);
    }
    onChangePwdModalChange(true);
  };
...
EditPassword.propTypes = {
  ...
  editPwd: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { editPwd })(EditPassword);
